...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_events_status_start_time 
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_event_registrations_user_event 
ON public.event_registrations (user_id, event_id);

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_social_posts_user_status_created 
ON public.social_posts (user_id, status, created_at DESC) 
WHERE status = 'active';

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_challenges_status_dates 
...
-- Create materialized view for better performance on frequently accessed data
...
SELECT 
  ...
  COUNT(DISTINCT cp.challenge_id) as total_challenges_joined,
  COUNT(DISTINCT sp.id) as total_posts_created,
  COALESCE(SUM(lt.points) FILTER (WHERE lt.type = 'earned'), 0) as total_points_earned,
  MAX(sp.created_at) as last_post_date,
  MAX(er.registered_at) as last_event_registration
FROM public.profiles p
LEFT JOIN public.event_registrations er ON p.id = er.user_id
LEFT JOIN public.challenge_participations cp ON p.id = cp.user_id
LEFT JOIN public.social_posts sp ON p.id = sp.user_id AND sp.status = 'active'
LEFT JOIN public.loyalty_transactions lt ON p.id = lt.user_id
