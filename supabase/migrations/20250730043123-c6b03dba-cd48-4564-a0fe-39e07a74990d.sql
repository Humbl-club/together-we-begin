...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    'message', format('Successfully moderated %s %s(s)', updated_count, content_type_param)
  );
END;
$$;
...
BEGIN
  ...
  WITH posts_with_reports AS (
    ...
    WHERE (content_type_filter = 'all' OR content_type_filter = 'post')
  ...
  comments_with_reports AS (
    ...
    WHERE (content_type_filter = 'all' OR content_type_filter = 'comment')
