...
-- This creates the core organization structure for multi-tenancy
...
CREATE TABLE IF NOT EXISTS organizations (
  ...
  name TEXT NOT NULL,
  slug TEXT UNIQUE NOT NULL, -- URL-friendly identifier (e.g., 'yoga-club')
...
CREATE TABLE IF NOT EXISTS organization_features (
  ...
  -- Billing
  price_override DECIMAL(10,2), -- Custom pricing if applicable
...
BEGIN
  ...
  -- If not, return their first organization
  RETURN (
    SELECT organization_id 
    FROM organization_members 
    WHERE user_id = auth.uid()
